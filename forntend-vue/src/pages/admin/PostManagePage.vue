    ...
    <template v-else>
      <div class="mobile-container">
        ...
        <div class="mobile-search-bar">
          ...
          <a-select
            v-model:value="searchParams.status"
            class="mobile-status-select-minimal"
            placeholder="筛选状态"
            :options="POST_STATUS_OPTIONS"
            allow-clear
          >
        ...
        <!-- 帖子列表 -->
        <div class="mobile-post-container">
          <div v-if="loading" class="mobile-loading">
            <a-spin />
          </div>
          <div v-else-if="dataList.length === 0" class="mobile-empty">
            <a-empty description="暂无帖子数据" />
          </div>
          <div v-else class="post-cards-list">
            <div v-for="post in dataList" :key="post.id" class="post-card">
              <div class="card-header">
                <div class="author-info">
                  <a-avatar :size="24" :src="post.user?.userAvatar" />
              ...
              <div class="card-content" @click="handleView(post)">
                <h3 class="post-title">{{ post.title }}</h3>
                <div class="post-meta-tags">
                  <span v-if="post.category" class="category-text"># {{ post.category }}</span>
                  <span class="post-time">{{ dayjs(post.createTime).format('MM-DD HH:mm') }}</span>
                </div>
                <div v-if="post.attachments && post.attachments.length" class="post-attachments">
                  <div class="attachment-count">
                    <PictureOutlined /> {{ post.attachments.length }} 张图片
                  </div>
                </div>
              </div>
              <div class="card-footer">
                ...
                <div class="card-actions">
                  ...
                  >拒绝</a-button>
                  <a-dropdown>
                    <a-button size="small" class="btn-more">更多</a-button>
                    <template #overlay>
                      <a-menu>
                        <a-menu-item @click="handleView(post)">查看详情</a-menu-item>
                        <a-menu-item @click="showDeleteConfirm(post)" danger>彻底删除</a-menu-item>
                      </a-menu>
                    </template>
                  </a-dropdown>
                </div>
        ...
        <!-- 移动端分页 -->
        <div class="mobile-pagination-minimal" v-if="device !== DEVICE_TYPE_ENUM.PC">
          <div class="page-current">
            第 {{ searchParams.current }} / {{ Math.ceil(total / searchParams.pageSize) }} 页
          </div>
          <div class="page-nav">
            <a-button
