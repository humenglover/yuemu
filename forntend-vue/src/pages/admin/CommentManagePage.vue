  ...
  <div id="commentManagePage">
        ...
        <div class="search-and-button-container">
          <a-form layout="inline" :model="searchParams" @finish="doSearch">
            ...
            <a-form-item label="评论状态">
              <a-select v-model:value="searchParams.isDelete" placeholder="选择评论状态">
        ...
        <div class="mobile-content">
          ...
          <div class="comment-list">
            <van-checkbox-group v-model="state.selectedRowKeys">
              <van-cell-group inset v-for="comment in dataList" :key="comment.commentId">
                <van-card class="comment-card">
                  <template #title>
                    <div class="comment-info">
                      ...
                      <div class="comment-meta">
                        <div class="meta-item">
                          ...
                          <span class="meta-value">{{ comment.commentId }}</span>
                        </div>
                        <div class="meta-item">
                          <span class="meta-label">用户ID：</span>
                          <span class="meta-value">{{ comment.userId }}</span>
                        </div>
                        <div class="meta-item">
                          <span class="meta-label">目标ID：</span>
                          <span class="meta-value">{{ comment.targetId }}</span>
                        </div>
                        <div class="meta-item">
                          <span class="meta-label">目标类型：</span>
                          <span class="meta-value">{{ comment.targetType === 1 ? '图片' : '帖子' }}</span>
                        </div>
                        <div class="meta-item">
                          <span class="meta-label">创建时间：</span>
                          <span class="meta-value">{{ dayjs(comment.createTime).format('YYYY-MM-DD HH:mm:ss') }}</span>
                        </div>
                      </div>
                    </div>
                  </template>
                  ...
                  <template #tags>
                    <van-tag
                      :type="comment.isDelete === 0 ? 'success' : 'danger'"
                      round
                      size="small"
                      style="margin-right: 4px; padding: 0 8px"
                    >
                      {{ comment.isDelete === 0 ? '正常' : '已删除' }}
                    </van-tag>
                  </template>
    ...
    >
      <div class="delete-confirm-content">
        ...
        <h3 class="confirm-title">确认删除该评论？</h3>
        <p class="confirm-desc">
          评论内容：{{ selectedComment?.content || '无内容' }}<br>
          创建时间：{{ selectedComment ? dayjs(selectedComment.createTime).format('YYYY-MM-DD HH:mm:ss') : '未知' }}
        </p>
