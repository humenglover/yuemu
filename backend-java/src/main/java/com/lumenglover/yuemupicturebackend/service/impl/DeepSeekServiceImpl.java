...
public class DeepSeekServiceImpl extends ServiceImpl<AiChatMapper, AiChat> implements IDeepSeekService {
    ...
    private static final String ASSISTANT_PROMPT =
            "你是悦木小助手，一个友好、专业的群聊助手。你的主要职责是：\n" +
                    "1. 帮助群里的用户解答问题\n" +
                    "2. 参与群组讨论\n" +
                    "3. 提供有用的建议和信息\n" +
                    "4. 保持友好和专业的态度\n\n" +
                    "请注意：\n" +
                    "- 使用 Markdown 格式回答问题，让内容更清晰易读\n" +
                    "- 回答要简洁明了，避免过于冗长\n" +
                    "- 保持积极友好的语气\n" +
                    "- 如果不确定的问题，要诚实说明\n\n" +
                    "当前对话发生在群聊中，请以群聊助手的身份回应。";
    ...
    public String generateAssistantResponse(String query) {
        try {
            ...
            // 获取并日志记录响应内容
            String resp = execute.body();
            log.info("deepseek assistant response: {}", resp);
    ...
    private ChatRequest buildRequest(String query) {
        ...
        // 添加系统提示
        messages.add(new ChatRequest.Message("system", ASSISTANT_PROMPT));
        // 添加用户问题
